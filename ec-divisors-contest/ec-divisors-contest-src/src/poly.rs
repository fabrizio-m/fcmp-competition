...
impl<F: From<u64> + Zeroize + PrimeField> Poly<F> {
    ...
    fn shift_by_y(mut self, power_of_y: usize) -> Self {
        ...
        core::mem::swap(
            &mut self.yx_coefficients[power_of_y - 1],
...
impl<F: From<u64> + Zeroize + PrimeField> Poly<F> {
    ...
    fn leading_coefficient(&self) -> (usize, usize) {
        if self.y_coefficients.len() > self.yx_coefficients.len() {
            ...
            (
                self.yx_coefficients.len(),
                self.yx_coefficients.last().unwrap().len(),
            )
        } else {
        ...
        for (y_pow_sub_one, coeff) in self.y_coefficients.iter().enumerate() {
            ...
            leading_coefficient = <_>::conditional_select(
                ...
                coeff_is_non_zero
                    ...
                    & (potential.ct_gt(greater_than_or_equal)
                        | potential.ct_eq(greater_than_or_equal)),
            );
        }
    ...
    pub(crate) fn div_rem(self, denominator: &Self) -> (Self, Self) {
        ...
        fn conditional_select_poly<F: From<u64> + Zeroize + PrimeField>(
            ...
            for (a, b) in a.y_coefficients.iter().zip(&b.y_coefficients) {
                res.y_coefficients
                    .push(<_>::conditional_select(a, b, choice));
            }
        ...
        if denominator_leading_coefficient == (0, 0) {
            return (
                self * denominator.zero_coefficient.invert().unwrap(),
                Poly::zero(),
            );
        }
        ...
        for _ in 0..iterations {
            ...
            let meaningful_iteration = numerator_coefficient
                .ct_gt(&denominator_dividing_coefficient)
            ...
            let proper_powers_of_yx = CoefficientIndex {
                y_pow: numerator_coefficient
                    .y_pow
                    .wrapping_sub(denominator_dividing_coefficient.y_pow),
                x_pow: numerator_coefficient
                    .x_pow
                    .wrapping_sub(denominator_dividing_coefficient.x_pow),
            };
            ...
            quotient =
                conditional_select_poly(quotient, quotient_if_meaningful, meaningful_iteration);
        ...
        if denominator_leading_coefficient != (CoefficientIndex { y_pow: 0, x_pow: 0 }) {
            while {
                let index = CoefficientIndex {
                    y_pow: remainder.y_coefficients.len().try_into().unwrap(),
                    x_pow: 0,
                };
                bool::from(
            ...
            while {
                let index = CoefficientIndex {
                    y_pow: 0,
                    x_pow: remainder.x_coefficients.len().try_into().unwrap(),
                };
                bool::from(
                    index.ct_gt(&denominator_leading_coefficient)
                        | index.ct_eq(&denominator_leading_coefficient),
