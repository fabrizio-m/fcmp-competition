...
pub fn new_divisor_old<C: DivisorCurve>(points: &[C]) -> Option<Poly<C::FieldElement>> {
    ...
    while let Some(a) = iter.next() {
        ...
        divs.push((
            2,
            a + b.unwrap_or(C::identity()),
            line::<C>(a, b.unwrap_or(-a)),
        ));
    }
    ...
    while divs.len() > 1 {
        ...
        while let Some((a_points, a, a_div)) = divs.pop() {
            ...
            let numerator = a_div
                .mul_mod(&b_div, &modulus)
                .mul_mod(&line::<C>(a, b), &modulus);
    ...
    impl crate::DivisorCurve for EdwardsPoint {
        ...
        fn to_xy(point: Self) -> Option<(Self::FieldElement, Self::FieldElement)> {
            ...
            let wei_x = (edwards_y_plus_one * one_minus_edwards_y.invert().unwrap())
                + (Self::FieldElement::from(486662u64)
                    * Self::FieldElement::from(3u64).invert().unwrap());
            let c = (-(Self::FieldElement::from(486662u64) + Self::FieldElement::from(2u64)))
                .sqrt()
                .unwrap();
            let wei_y =
                c * edwards_y_plus_one * (one_minus_edwards_y * edwards_x).invert().unwrap();
            Some((wei_x, wei_y))
        }
    }
}
